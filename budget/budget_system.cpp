...
PureIncome BudgetSystem::ComputeIncome(const Date& from, const Date& to) const {
  auto accumulate_income =
      [this](const std::map<Date, PureIncome>& transaction_history,
             const Date& from, const Date& to) -> PureIncome {
    PureIncome res{0};
  ...
  return accumulate_income(incomes_, from, to) -
         accumulate_income(spendings_, from, to);
}
...
std::map<Date, PureIncome>::iterator BudgetSystem::AddBoundDate(
  ...
  if (auto it = transaction_history.find(date);
      it != std::end(transaction_history)) {
    return it;
  }
...
void BudgetSystem::UpdateTransactions(const Date& from, const Date& to,
                                      IncomeValue value, Transaction tr) {
